...
type PcCanvasMouseState struct {
	PreviousCoord *fyne.PointEvent // Fixed the typo in field name
}

type PxCanvas struct {
	widget.BaseWidget
	apptype.PxCanvasConfig
	Renderer    *PxCanvasRenderer // Changed field name to Renderer
	PixlData    image.Image
	MouseState  PcCanvasMouseState // Fixed the typo in type name
...
func NewPxCanvas(state *apptype.State, config apptype.PxCanvasConfig) *PxCanvas {
	...
	pxCanvas.PixlData = NewBlankImage(pxCanvas.PxCols, pxCanvas.PxRows, color.NRGBA{128, 128, 128, 255})
	pxCanvas.ExtendBaseWidget(pxCanvas) // Changed ExtendBaseWidget argument to pxCanvas
	return pxCanvas
}

func (pxCanvas *PxCanvas) CreateRenderer() fyne.WidgetRenderer {
	canvasImage := canvas.NewImageFromImage(pxCanvas.PixlData)
...
func (pxCanvas *PxCanvas) SetColor(c color.Color, x, y int) {
	if nrhba, ok := pxCanvas.PixlData.(*image.NRGBA); ok {
		nrhba.Set(x, y, c)
	}

	if rgba, ok := pxCanvas.PixlData.(*image.RGBA); ok {
		rgba.Set(x, y, c)
	}
	pxCanvas.Refresh()
}
...
func (pxCanvas *PxCanvas) LoadImage(img image.Image) {
	...
	pxCanvas.PxCanvasConfig.PxCols = dimensions.Dy()

	pxCanvas.PixlData = img
